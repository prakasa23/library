...
app.post('/submit', (req, res) => {
    ...
    db.run("INSERT INTO records (user, book, action) VALUES (?, ?, ?)", [user, book, action], () => {
        res.send(`<body style="background:#0f0c29; color:white; display:flex; justify-content:center; align-items:center; height:100vh; font-family:sans-serif; text-align:center;"><div><h2>✔ Action Recorded!</h2><p>Returning to dashboard...</p><script>setTimeout(()=>window.location.href='/options', 2000)</script></div></body>`);
    });
});
...
// LISTEN ON 0.0.0.0 to allow other devices on Wi-Fi to connect
const PORT = process.env.PORT || 3000;
app.listen(PORT, '0.0.0.0', () => {
    console.log(`Server is running!`);
    console.log(`Local access: http://localhost:${PORT}`);
    console.log(`Network access: Check your IP (e.g., http://192.168.x.x:${PORT})`);
});
